...
public class AnimationPathEditor : EditorWindow
{
	...
	private int CountAffectedProperties(ref int affectedAnimations)
	{
		...
		if (animationObject is AnimatorController animatorController)
		{
			foreach (var layer in animatorController.layers)
			{
				...
				{
					// Collect all clips (including those in BlendTrees)
					var foundClips = new System.Collections.Generic.List<AnimationClip>();
					GetAllAnimationClips(state.state.motion, foundClips);

					// Count properties in all clips encountered
	...
	// Recursively gather all AnimationClips from any Motion (AnimationClip or BlendTree).
	private void GetAllAnimationClips(Motion motion, System.Collections.Generic.List<AnimationClip> foundClips)
	{
		if (motion == null) return;

		// If it's a direct clip, add it
		if (motion is AnimationClip clip)
		{
			foundClips.Add(clip);
		}
		// If it's a blend tree, traverse its children
		else if (motion is BlendTree blendTree)
		{
			foreach (var child in blendTree.children)
			{
				GetAllAnimationClips(child.motion, foundClips);
			}
		}
	}
}
